...
/* USER CODE BEGIN 1 */
/*使用HAL库串口全双工通信记住个原则，不管是哪种工作模式，都不要同时收发数据，
__HAL_LOCK(huart) ，锁的是整个外设，不是TX通道或RX通信，非HAL库无影响，如果以
后串口出现锁死的话，再想想我说的话，或许有点帮助。串口发送前检测下发送状态标
志，串口中断函数中，先暂停下发送通道，处理数据，再恢复发送通道。*/
void usart_sendstring(uint8_t *str)
{
    while (HAL_UART_Transmit_DMA(&huart1, str, strlen((const char *)str) + 10) != HAL_OK);
}
